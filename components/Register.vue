...
const createAccount = async (e: Event) => {
    ...
    try {
        ...
        await createUserWithEmailAndPassword(nuxtApp.$auth, email.value, password.value)
    } catch (error) {
        console.error(error);
    }
}
...
<template>
    <form class="max-w-xl mx-auto py-5 flex flex-col gap-3" @submit="createAccount">
        ...
        <button type="submit" class="bg-slate-500 text-white py-2 px-5 rounded-lg cursor-pointer">Register</button>
    </form>
</template>
<style scoped>
input {
